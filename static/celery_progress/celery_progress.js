class CeleryProgressBar {
    ...
    onSuccessDefault(progressBarElement, progressBarMessageElement, result) {
        ...
        progressBarMessageElement.textContent = "Επιτυχία! " + result;
    }
    ...
    onErrorDefault(progressBarElement, progressBarMessageElement, excMessage, data) {
        ...
        progressBarMessageElement.textContent = "Ωχ, κάτι πήγε στραβά! " + excMessage;
    }
    ...
    onRetryDefault(progressBarElement, progressBarMessageElement, excMessage, retryWhen) {
        ...
        let message = 'Επανάληψη' + Math.round((retryWhen.getTime() - Date.now())/1000) + 's: ' + excMessage;
        this.onError(progressBarElement, progressBarMessageElement, message);
    }
    ...
    onIgnoredDefault(progressBarElement, progressBarMessageElement, result) {
        ...
        progressBarMessageElement.textContent =  result || 'Το αποτέλεσμα της εργασίας αγνοήθηκε!'
    }
    ...
    onProgressDefault(progressBarElement, progressBarMessageElement, progress) {
        ...
        if (progress.current == 0) {
            if (progress.pending === true) {
                progressBarMessageElement.textContent = 'Αναμονή για έναρξη της εργασίας ...';
            } else {
                progressBarMessageElement.textContent = 'Η εργασία ξεκίνησε ...';
            }
        } else {
            progressBarMessageElement.textContent = progress.current + ' από ' + progress.total + ' ολοκληρώθηκαν. ' + description;
        }
    }
        ...
        if (data.complete === true) {
            ...
            if (data.success === true) {
                ...
                if (data.state === 'IGNORED') {
                    ...
                    this.onDataError(this.progressBarElement, this.progressBarMessageElement, "Σφάλμα δεδομένων");
                }
            }
    ...
    async connect() {
        ...
        try {
            ...
            this.onNetworkError(this.progressBarElement, this.progressBarMessageElement, "Σφάλμα δικτύου");
            throw networkError;
        }
        ...
        if (response.status === 200) {
            ...
            try {
                ...
                this.onDataError(this.progressBarElement, this.progressBarMessageElement, "Σφάλμα ανάλυσης")
                throw parsingError;
            }
